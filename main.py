...
def main():
    ...
    for x in os.listdir("videos"):
        if x.endswith(".mp4"):
            ...
            counter = 0
            while video.isOpened() and counter < 10:
                preReadTime = time.time()
                ret, image = video.read()
                readTime = time.time() - preReadTime
                print("readTime is " + str(readTime))
                # Resize and pad the image to keep the aspect ratio and fit the expected size.
                preResizeTime = time.time()
                input_image = tf.expand_dims(image, axis=0)
                input_image = tf.image.resize_with_pad(input_image, input_size, input_size)
                resizeTime = time.time() - preResizeTime
                print("resizeTime is " + str(resizeTime))
